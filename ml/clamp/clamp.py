...
def get_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="music",
        help="The query modal, either 'music' or 'text'. If 'music', the input is \"music_query.mxl\", if 'text', the input is \"text_query.txt\"",
    )
    parser.add_argument(
        "-key_modal",
        type=str,
        default="text",
        help="The key modal, either 'music' or 'text'. If 'music', the inputs are all mxl in the \"music_keys\" folder, if 'text', the inputs are all lines in the \"text_keys.txt\" file",
    )
    parser.add_argument(
        "-top_n", type=int, default=10, help="The number of top results to return"
    )
    ...
    if modal == "music":
        ...
        for item in data:
            text_encodings = tokenizer(
                item, return_tensors="pt", truncation=True, max_length=TEXT_LENGTH
            )
...
def load_music(filename):
    ...
    p = subprocess.Popen(
        [
            "/home/finlay/midi-ml/muzic/clamp/.venv/bin/python",
            ...
            "6",
            "-x",
            f"inference/music_keys/{filename}",
        ],
    ...
    with torch.no_grad():
        for ids in tqdm(ids_list):
            ...
            if modal == "text":
                ...
                features = model.text_enc(
                    ids.to(device), attention_mask=masks.to(device)
                )["last_hidden_state"]
                features = model.avg_pooling(features, masks)
                features = model.text_proj(features)
            else:
                masks = torch.tensor([1] * (int(len(ids[0]) / PATCH_LENGTH))).unsqueeze(
                    0
                )
...
if __name__ == "__main__":
    ...
    if KEY_MODAL == "music":
        ...
        if os.path.exists(
            "inference/cache/" + KEY_MODAL + "_key_cache_" + str(music_length) + ".pth"
        ):
            with open(
                "inference/cache/"
                + KEY_MODAL
            ...
            cached_keys = [
                key for i, key in enumerate(cached_keys) if i not in files_to_remove
            ]
            cached_key_filenames = [
                filename
                for i, filename in enumerate(cached_key_filenames)
                if i not in files_to_remove
            ]
            cached_key_features = [
                feature
                for i, feature in enumerate(cached_key_features)
                if i not in files_to_remove
            ]
            ...
            key_filenames = [
                ...
                if filename not in cached_key_filenames
            ]
