...
class Overseer:
        ...
        try:
            while True:
                ...
                if self.recording_ready_event.is_set():
                    # get recording
                    recording_path = os.path.join(self.record_dir, self.listener.outfile)
                    console.log(f"{self.p} triggering playback from recording '{recording_path}'")
    ...
    def change_tempo(self, midi_file_path: str) -> str:
        ...
        if not tempo_added:
            ...
            console.log(f"{self.p} [red]adding message to new track {new_message}", markup=True)
