...
def gen_transformations(filename: str, options) -> list[str]:
    ...
    for opt in options:
        transformed_filenames.append(f"{filename[:-4]}_t{opt[0]:02d}s{opt[1]:02d}{filename[-4:]}")
...
def calc_sims(rows: list[str], all_rows: list[str],  index: int):
    ...
    with progress:
        for i, pf_row in enumerate(all_rows[:1]):
            ...
            row_embedding = model.run([pf_row]) # (1, 768)
            ...
            for pf_col in all_rows:
                ...
                if pf_col == pf_row:
                    r.json().set(f'cmp:{row_name}:{row_name}', "$", {"transpose": 0, "shift": 0}, nx=True)
                    r.json().set(f'cmp:{row_name}:{row_name}', "$", {"clamp": 1.0})
                    continue

                transformed_columns = gen_transformations(pf_col.replace("play", "train"), mod_table)
                col_embeddings = model.run(transformed_columns) # (96, 768)
                ...
                # )
                similarities = [1 - cosine(row_embedding, col_embedding) for col_embedding in col_embeddings]
                ...
                r.json().set(f'cmp:{row_name}:{best_match[:-7]}', "$", {"transpose": best_trans, "shift": best_shift, "clamp": best_sim})
