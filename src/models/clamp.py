...
def load_music(pf_file: str) -> str:
    ...
    p = subprocess.Popen(args=args, stdout=subprocess.PIPE)  # type: ignore
...
def encoding_data(data: list[str], music_length) -> list[torch.Tensor]:
    """
    ...
    for item in track(data, "encoding..."):
        patches = MusicPatchilizer().encode(
...
def get_features(model, ids_list: list) -> list[torch.Tensor]:
    """
...
def get_embedding(model, file: str):
    # keys, filenames = convert(files)
    key = load_music(file)
    id = encoding_data([key], music_length=model.config.max_length)
    return get_features(model, id)
...
class CLaMP(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        local_dir = os.path.join("src", "models", "configs", "clamp")
        ...
        if not os.path.exists(local_dir):
            ...
            with requests.get(config_url, stream=True) as r:
                ...
                with open(local_dir + "/config.json", "wb") as f:
                    for chunk in track(
            ...
            with requests.get(model_url, stream=True) as r:
                r.raise_for_status()
                total_size = int(r.headers.get("content-length", 0))
                with open(local_dir + "/pytorch_model.bin", "wb") as f:
                    for chunk in track(
                        r.iter_content(chunk_size=chunk_size),
