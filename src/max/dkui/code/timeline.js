...
function linearInterpolation(arr, point) {
  ...
  var returned_value;
  if (!initialPoint && initialPoint != 0) return arr[arr.length - 1].values;
  if (!arr[initialPoint - 1]) return -1;
  ...
  var returned_value =
    (p0.values * (p1.times - point) + p1.values * (point - p0.times)) /
    (p1.times - p0.times);
    ...
    if (playMode) {
      ...
      for (var i = 0; i < itemArr.length; i++) {
        for (var j = 0; j < itemArr[i].elements.length; j++) {
          if (itemArr[i].elements[j].times > cursor) set_source_rgba(color2);
          else set_source_rgba(color1);
          if (itemArr[i].elements[j].times > 0) {
            rectangle(
              (itemArr[i].elements[j].times / max_length) * (width - 30) + 15,
              40,
              1,
              (height - 55) / 2
            );
      ...
      for (var i = 0; i < itemArr.length; i++) {
        ...
        for (var j = 0; j < itemArr[i].elements.length; j++) {
          ...
          ellipse(
            15 +
              (itemArr[i].elements[j].times / 30000) * (width - 35) -
              (cursor / 30000) * (width - 35),
            63 + 46 * i,
            5,
            5
          );
          ...
          if (
            itemArr[i].elements[j].interpolation &&
            itemArr[i].elements[j + 1]
          ) {
            rectangle(
              25 +
                (itemArr[i].elements[j].times / 30000) * (width - 35) -
                (cursor / 30000) * (width - 35),
              65 + 46 * i,
              (itemArr[i].elements[j + 1].times / 30000) * (width - 35) -
                (itemArr[i].elements[j].times / 30000) * (width - 35) -
                20,
              1
            );
...
function closePatcher(patch, subpatch, subpatch2, inletToDelete) {
  ...
  var closezl = patch.newdefault(
    this.box.rect[0],
    this.box.rect[1],
    "zl.reg",
    "close"
  );
  var closepcontrol = patch.newdefault(
    this.box.rect[0],
    this.box.rect[1],
    "pcontrol"
  );
      ...
      for (var j = 0; j < itemArr[i].elements.length; j++) {
        ...
        if (x > x_min && x < x_max && y > y_min && y < y_max) {
          if (max.ctrlkeydown) {
            ...
            if (this_time) markersArr.splice(this_time, 1);
            itemArr[i].elements.splice(j, 1);
            if (itemArr[i].elements.length == 0) {
              itemArr.splice(i, 1);
              this.box.size(width, height - 46);
              if (itemArr.length == 0) play_mode();
              mgraphics.redraw();
              break;
            }
          } else if (max.shiftkeydown) {
            itemArr[i].elements[j].isMarked = itemArr[i].elements[j].isMarked
              ? false
              : true;
            if (itemArr[i].elements[j].isMarked) {
              markersArr.push(itemArr[i].elements[j].times);
              markersArr.sort(function (a, b) {
                return a - b;
              });
            } else {
              var this_time = markersArr.indexOf(itemArr[i].elements[j].times);
        ...
        } else itemArr[i].elements[j].isSelected = false;
      }
    }
    mgraphics.redraw();
  }
...
function createMess(index1, index2) {
  ...
  if (!editMsg) {
    editMsg = this.patcher.newdefault(
      this.box.rect[2] + 20,
      this.box.rect[1] + 40,
      "message"
    );
...
function ondrag(x, y, but, cmd, shift, capslock, option, ctrl) {
  if (allowDrag) {
    ...
    set_cursor(
      Math.max(
        Math.min(
          init_curs + (init_x - x) * 30 * (max_length / 60000),
          max_length
        ),
        0
      )
...
function edit() {
  ...
  if (this_time > max_length) set_max_length(this_time);
  var a = string.shift();
  if (a != "interpolation") {
    error("edit message with incorrect format \n");
    return;
  ...
  itemArr[index1].elements.sort(function (a, b) {
...
function writeData() {
  ...
  if (!savePatch) {
    savePatch = this.patcher.newdefault(
      this.box.rect[2] + 20,
      this.box.rect[1],
      "p",
      "DO NOT DELETE"
...
function anything() {
  ...
  var selector = -1;
  for (var i = 0; i < itemArr.length; i++) {
    if (itemArr[i].object == sendName) selector = i;
  }
  if (selector == -1) {
    this_item = new Item();
    ...
    itemArr[selector].elements.sort(function (a, b) {
      return a.times - b.times;
    });
  }
  if (!playMode) {
...
function msg_int(i) {
  if (i < markersArr.length && i >= 0) set_cursor(markersArr[i]);
}
...
function load() {
  ...
  while (string.length > 0) {
    ...
    if (times > max_length) set_max_length(times);
    var valueArr = [];
    ...
    if (this_element.isMarked) markersArr.push(this_element.times);
    markersArr.sort(function (a, b) {
      return a - b;
    });
