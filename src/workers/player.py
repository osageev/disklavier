...
class Player(Worker):
    ...
    def play(self, queue: PriorityQueue):
        ...
        while queue.qsize() > 0:
            ...
            if self.verbose:
                console.log(
                    f"{self.tag} \tit is {td_now.strftime('%H:%M:%S.%f')[:-3]} and the last note was played at {self.td_last_note.strftime('%H:%M:%S.%f')[:-3]}. I will sleep for {dt_sleep.total_seconds()}s"
                )
